...
from .config import LLM_CONFIG, PROFILE_PATH, STORAGE_CONFIG, validate_config
...
def build_engine():
    ...
    chat_store = SimpleChatStore.from_persist_path(str(STORAGE_CONFIG["store_path"]))
    ...
    chat_engine = SimpleChatEngine.from_defaults(llm=llm, system_prompt=prompt, memory=memory)
